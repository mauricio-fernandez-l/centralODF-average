...
def pbasis_random(n, r_plus=0.3, info=False):
    '''
    pbasis_random(n,r_plus) returns a permutation basis for the space of n-th-order
    isotropic tensors.
    '''
    d = tn.diso(n)
    B = tn.Biso(n)
...
def pbasis_random_det(n, info=False):
    '''
...
def tc(odata, r):
    ...
    honb = [np.loadtxt('src/data/honb' + str(a) + '.txt')
